          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              ...
              <div>
                ...
                <p class="text-blue-100">Update guest details</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="flex pt-6 space-x-4 border-t">
              ...
              >
                ...
                {{ form.processing ? 'Updating Guest...' : 'Update Guest' }}
              </button>
...
const form = useForm({
  ...
  is_attending: props.guest?.is_attending || false,
});
...
const validatePhone = () => {
  const phoneRegex = /^(0[0-9]{9}$|\+251[0-9]{9}$)/;
  const phoneValue = form.phone || '';
  if (phoneValue && !phoneRegex.test(phoneValue)) {
    phoneError.value = 'Phone must be 10 digits starting with 0 (e.g., 0912345678) or 13 digits starting with +251 (e.g., +251912345678)';
  } else {
    phoneError.value = '';
  }
};
...
const handleInput = (event) => {
  form.phone = event.target.value.replace(/[^0-9+]/g, '');
  validatePhone();
};
...
function submit() {
  ...
  form.clearErrors();
  form.put(route('guest.update', props.guest.id), {
    onSuccess: () => {
      successMessage.value = 'Guest updated successfully!';
      router.visit(route('guest.show', props.guest.id));
    },
    onError: () => {
