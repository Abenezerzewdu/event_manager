        ...
        <div class="px-6 py-4 bg-gradient-to-r from-blue-500 to-blue-600">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              ...
              <div>
                <h1 class="text-2xl font-bold text-white">Edit Vendor Service</h1>
                <p class="text-blue-100">Update the vendor service details</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="flex pt-6 space-x-4 border-t">
              ...
              >
                ...
                {{ form.processing ? 'Updating Vendor Service...' : 'Update Vendor Service' }}
              </button>

              <Link
                :href="route('vendor-service.show', vendorService.id)"
...
const form = useForm({
  vendor_id: props.vendorService?.vendor_id || '',
  service_id: props.vendorService?.service_id || '',
});

function submit() {
  form.clearErrors();
  form.put(route('vendor-service.update', props.vendorService.id), {
    onSuccess: () => {
      successMessage.value = 'Vendor Service updated successfully!';
      router.visit(route('vendor-service.show', props.vendorService.id));
    },
    onError: () => {
