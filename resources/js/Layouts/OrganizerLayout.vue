<script setup>
import { ref, onMounted, onBeforeUnmount, watch } from "vue";
import { Link, useForm } from "@inertiajs/vue3";
import {
    Home,
    Calendar,
    Users,
    ShoppingBag,
    Sun,
    Moon,
    LogOut,
} from "lucide-vue-next";

const isOpen = ref(false);
const isDark = ref(false);

// Example user (replace with Inertia props)
const user = { name: "Organizer Name" };

// Logout form
const logoutForm = useForm({});
const handleLogout = () => logoutForm.post("/logout");

// Click outside sidebar
const handleClickOutside = (event) => {
    if (!event.target.closest("aside") && !event.target.closest("button")) {
        isOpen.value = false;
    }
};
onMounted(() => document.addEventListener("click", handleClickOutside));
onBeforeUnmount(() =>
    document.removeEventListener("click", handleClickOutside)
);

// Dark mode toggle
watch(isDark, (val) => {
    if (val) document.documentElement.classList.add("dark");
    else document.documentElement.classList.remove("dark");
});
</script>

<template>
    <div class="flex flex-col h-screen font-sans bg-gray-100 dark:bg-gray-900">
        <!-- Topbar -->
        <div
            class="h-12 flex items-center justify-end px-6 bg-gradient-to-r from-blue-800 to-indigo-900 dark:from-gray-900 dark:to-indigo-800 text-white shadow"
        >
            <button
                @click="isDark = !isDark"
                class="flex items-center gap-2 px-3 py-1 rounded-lg hover:bg-white/10 transition"
            >
                <Sun v-if="!isDark" class="w-5 h-5 text-yellow-400" />
                <Moon v-else class="w-5 h-5 text-blue-300" />
                <span class="hidden sm:inline text-sm">
                    {{ isDark ? "Dark" : "Light" }} Mode
                </span>
            </button>
        </div>

        <div class="flex flex-1 overflow-hidden">
            <!-- Sidebar -->
            <aside
                :class="[
                    'bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 w-64 p-5 shadow-xl transform transition-transform duration-300 ease-in-out z-40',
                    isOpen ? 'translate-x-0' : '-translate-x-64',
                    'fixed md:static md:translate-x-0 rounded-r-2xl flex flex-col justify-between',
                ]"
            >
                <div>
                    <!-- Brand / Home -->
                    <div class="mb-6">
                        <Link
                            href="/"
                            class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-blue-500 transition text-sm text-black-100"
                        >
                            <Home class="w-5 h-5" />
                            <span>evento 🎈</span>
                        </Link>
                    </div>

                    <h2 class="text-xl font-bold mb-8 text-center">
                        Organizer Panel
                    </h2>

                    <!-- Sidebar Links -->
                    <ul class="space-y-3">
                        <li>
                            <Link
                                href="/organizer/dashboard"
                                class="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-blue-500 transition"
                            >
                                <Home class="w-5 h-5" /> Overview
                            </Link>
                        </li>
                        <li>
                            <Link
                                href="/organizer/events"
                                class="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-blue-500 transition"
                            >
                                <Calendar class="w-5 h-5" /> My Events
                            </Link>
                        </li>
                        <li>
                            <Link
                                href="/my-orders"
                                class="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-yellow-500 hover:text-black transition"
                            >
                                <ShoppingBag class="w-5 h-5" /> My Orders
                            </Link>
                        </li>
                        <li>
                            <Link
                                href="/organizer/vendors"
                                class="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-blue-500 transition"
                            >
                                <Users class="w-5 h-5" /> Vendors
                            </Link>
                        </li>
                    </ul>
                </div>

                <!-- Bottom Section -->
                <div class="mt-6">
                    <button
                        @click="handleLogout"
                        class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-red-500/80 hover:text-white transition w-full text-left text-red-200"
                    >
                        <LogOut class="w-5 h-5" />
                        <span>Logout</span>
                    </button>

                    <div
                        class="mt-6 pt-4 border-t border-white/30 text-sm text-center"
                    >
                        <p>Logged in as:</p>
                        <p class="font-semibold text-lg">{{ user.name }}</p>
                    </div>
                </div>
            </aside>

            <!-- Main Content -->
            <div class="flex-1 flex flex-col">
                <!-- Mobile Toggle -->
                <header
                    class="flex items-center justify-between bg-white dark:bg-gray-800 shadow px-6 py-4 md:hidden"
                >
                    <button
                        class="text-blue-600 dark:text-blue-400 font-bold text-2xl"
                        @click.stop="isOpen = !isOpen"
                    >
                        ☰
                    </button>
                </header>

                <!-- Page slot -->
                <main
                    class="p-6 flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100"
                >
                    <slot />
                </main>
            </div>
        </div>
    </div>
</template>
